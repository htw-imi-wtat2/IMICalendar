...
RSpec.describe Category, type: :model do
  it 'converts a list of categories into an alphabetically sorted string' do
    ...
    expect(Category.categories_to_s(categories)).to eq("A, B, C, D")
  end
  it 'converts a string to an array of categories, creating missing ones' do
    ...
    categories = Category.s_to_categories('b, h, x, d')
    expect(Category.all.size).to eq 4
    expect(categories[0].name).to eq 'b'
    expect(Category.categories_to_s(categories)).to eq('b, d, h, x')
  end
